...
function DonutChart() {
  const r = 72, sw = 24, circ = 2 * Math.PI * r;
  let cumulative = 0;
  ...
  return (
    <div style={{ position: 'relative', width: 190, height: 190, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <svg width="190" height="190" style={{ position: 'absolute' }}>
        ...
        {segments.map(seg => (
          <circle key={seg.key} cx="95" cy="95" r={r} fill="none" stroke={seg.color} strokeWidth={sw - 2}
...
export default function Insights() {
  ...
  const totalMissed = SPEND_CATS.reduce((s, c) => s + c.missed, 0);
    ...
    <div style={{ height: '100%', overflowY: 'auto', padding: '56px 16px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ background: C.surface, borderRadius: 20, padding: '5px 10px', display: 'flex', gap: 5, alignItems: 'center', border: `0.5px solid ${C.border}` }}>
          ...
          <span style={{ color: C.textSec, fontSize: 12, fontWeight: 500 }}>May 2026</span>
        </div>
      </div>

      {/* Tab switcher */}
      <div style={{ display: 'flex', background: C.surface, borderRadius: 12, padding: 3, marginBottom: 22, border: `0.5px solid ${C.border}` }}>
        {(['spending', 'rewards'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: '9px', background: tab === t ? C.surfaceEl : 'none', borderRadius: 10, border: 'none', color: tab === t ? C.text : C.textSec, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {SPEND_CATS.map(cat => (
                <div key={cat.key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ color: C.text, fontSize: 13, fontWeight: 500, flex: 1 }}>{cat.emoji} {cat.label}</span>
          ...
          {totalMissed > 0 && (
            <div style={{ background: C.warningSoft, borderRadius: 14, padding: 14, display: 'flex', gap: 12, marginBottom: 22, border: `0.5px solid ${C.warning}44` }}>
              ...
              <div>
                ...
                <div style={{ color: C.textSec, fontSize: 13 }}>Switch cards on certain purchases to earn more</div>
              </div>
            </div>
          )}

          <div style={{ color: C.text, fontSize: 19, fontWeight: 700, marginBottom: 14, letterSpacing: -0.3 }}>Wallet+ Recommendations</div>
          {SPEND_CATS.filter(c => c.missed > 0).map(cat => {
            const best = CARDS.find(c => c.id === cat.bestCardId)!;
            return (
              <div key={cat.key} style={{ background: C.surface, borderRadius: 14, padding: 16, marginBottom: 10, border: `0.5px solid ${C.border}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  ...
                  <div style={{ background: C.accentSoft, borderRadius: 10, padding: '3px 8px', display: 'flex', gap: 3, alignItems: 'center' }}>
                    <span style={{ fontSize: 10 }}>✦</span>
                    <span style={{ color: C.wpMid, fontSize: 10, fontWeight: 700 }}>Tip</span>
                  </div>
                </div>
                <div style={{ color: C.textSec, fontSize: 13, lineHeight: 1.5 }}>
                  Use <span style={{ color: C.text, fontWeight: 600 }}>{best.issuer} {best.name}</span> for {cat.label.toLowerCase()} to earn <span style={{ color: C.text, fontWeight: 600 }}>{best.rewards[cat.key]}x points</span> — save <span style={{ color: C.text, fontWeight: 600 }}>${cat.missed.toFixed(2)}/mo</span>
                </div>
              </div>
            );
          })}
          ...
          {CARDS.map(card => {
            ...
            return (
              <div key={card.id} style={{ background: C.surface, borderRadius: 14, padding: 16, marginBottom: 10, border: `0.5px solid ${C.border}` }}>
                ...
                <div style={{ color: C.textSec, fontSize: 12 }}>${potential.toFixed(2)} potential with optimal card use</div>
              </div>
            );
          })}

          <div style={{ background: 'linear-gradient(135deg, rgba(123,47,190,0.2), rgba(74,144,217,0.1))', borderRadius: 16, padding: 18, display: 'flex', gap: 14, alignItems: 'center', marginTop: 8, border: `0.5px solid ${C.wpStart}44` }}>
            <span style={{ fontSize: 20 }}>✦</span>
            <div>
              <div style={{ color: C.text, fontSize: 14, fontWeight: 700, marginBottom: 3 }}>Wallet+ has saved you time</div>
              <div style={{ color: C.textSec, fontSize: 12 }}>23 smart card picks this month · avg. 0.4s decision time</div>
            </div>
          </div>
        </>
      )}
